...
function Assistant (options) {
  ...
  if (!options) {
    self.handleError_('Invalid options');
    return null;
  }
   ...
   * @private {string} valid api_version or API.ai if missing.
   */
  self.apiAi = false;
 ...
 *                 circulate back.
 */
Assistant.prototype.askForSignIn = function (actionPhrase, dialogState) {
 ...
 */
Assistant.prototype.newRuntimeEntity = function (name, items) {
...
Assistant.prototype.handleRequest = function (handler) {
  ...
  if (self.apiAi) {
    ...
    if (self.body_.conversation &&
      self.body_.conversation.conversation_token) {
      self.data = JSON.parse(this.body_.conversation.conversation_token);
    }
  }
