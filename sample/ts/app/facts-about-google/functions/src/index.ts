...
import { values, concat, random, randomPop } from './util'
...
interface TellFactParameters extends Parameters {
  category: string
}

interface FactContexts extends Contexts {
  [AppContexts.FACT]: Context<Parameters>
  [AppContexts.CATS]: Context<Parameters>
}
...
const app = dialogflow<ConvData, {}, FactContexts>({
  debug: true,
  init: () => ({
    data: {
...
app.intent<TellFactParameters>('tell_fact', (conv, { category }) => {
  ...
  if (!fact) {
    ...
    const parameters: TellFactParameters = {
      category: redirect,
    }
